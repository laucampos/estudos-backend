...
import { IDeleteUserInputDTO, IEditUserInputDTO, IGetUsersDBDTO, IGetUsersInputDTO, IGetUsersOutputDTO, ILoginInputDTO, ISignupInputDTO, User, USER_ROLES } from "../models/User"
    ...
    public updateUser = async (input: IEditUserInputDTO) => {
        ...
        if (payload.role === USER_ROLES.NORMAL){
            if (idToEdit !== payload.id){
                throw new Error("Não é possível editar outro usuário")
            }
        }
        ...
        if (!userDB) {
            throw new Error("Usuário a ser editado não encontrado")
        }
